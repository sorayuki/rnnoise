...
def remove_unused_opset_imports(model):
    ...
    existing_versions = {opset.domain: opset.version for opset in model.opset_import}
    del model.opset_import[:]
    for domain in [""] + sorted(domain for domain in used_domains if domain):
        version = existing_versions.get(domain, 13 if domain == "" else 1)
        model.opset_import.append(helper.make_operatorsetid(domain, version))


def optimize_model(input_path, output_path, optimization_level):
    code = """
import sys
...
session_options.graph_optimization_level = getattr(ort.GraphOptimizationLevel, f"ORT_ENABLE_{level_name.upper()}")
...
"""
    try:
        subprocess.run(
            [sys.executable, "-c", code, str(input_path), str(output_path), optimization_level],
            check=True,
        )
...
def make_rnn_graph(builder, weights):
...
def main():
    ...
    parser.add_argument(
        ...
        default="basic",
        help="ONNX Runtime graph optimization level used for the saved model",
    )
    ...
    builder = Builder()
    make_rnn_graph(builder, weights)

    inputs = [
        helper.make_tensor_value_info("features", TensorProto.FLOAT, [1, NB_FEATURES]),
    ...
    graph = helper.make_graph(builder.nodes, "rnnoise_rnn_streaming", inputs, outputs, initializer=builder.initializers)
    ...
    if args.no_optimize:
        ...
        try:
            ...
            optimize_model(raw_path, output_path, args.optimization_level)
        finally:
            try:
